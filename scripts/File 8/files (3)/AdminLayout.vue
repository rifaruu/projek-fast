...
import {
    ...
    Search, ChevronRight, Menu, X, History, QrCode,
...
const navGroups = computed(() => [
    ...
    },
    {
        ...
        items: [
            ...
            {
                key: 'letters.index', label: 'Semua Pengajuan', href: '/admin/surat',
                icon: ClipboardList, badge: notifCount.value || undefined, badgeColor: 'amber',
            },
            { key: 'approval', label: 'Antrian Approval', href: '/approval/dashboard', icon: CheckSquare },
...
<template>
    ...
    <div class="flex h-screen overflow-hidden bg-slate-50 font-sans">
        ...
        <Transition name="slide-sidebar">
            <aside
                v-show="showSidebar"
                class="fixed inset-y-0 left-0 z-30 flex flex-col border-r border-slate-200 bg-white transition-all duration-200 lg:relative lg:z-auto"
            ...
            >
                ...
                <nav class="flex-1 overflow-y-auto overflow-x-hidden px-2 py-2">
                    <template v-for="group in navGroups" :key="group.title">
                        <Transition name="fade-text">
                            <p v-if="sidebarExpanded" class="mb-1 mt-3 px-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 first:mt-1">
                                {{ group.title }}
                            </p>
                        </Transition>
        ...
        <div class="flex min-w-0 flex-1 flex-col overflow-hidden">
                    ...
                    <nav class="hidden items-center gap-1 text-sm md:flex">
                        ...
                        <template v-if="props.breadcrumbs?.length">
                            <template v-for="(bc, i) in props.breadcrumbs" :key="i">
                                <Link v-if="bc.href" :href="bc.href" class="text-slate-400 hover:text-slate-600 transition-colors">
                                    {{ bc.label }}
                                </Link>
                ...
                <div class="flex items-center gap-2">
                    <div class="relative hidden md:block">
                        ...
                        <input
                            ...
                            class="h-8 w-44 rounded-lg border border-slate-200 bg-slate-50 pl-8 pr-3 text-xs text-slate-700 outline-none transition focus:border-emerald-400 focus:bg-white focus:ring-2 focus:ring-emerald-100"
                        />
                    </div>
                    <button type="button" class="relative rounded-lg border border-slate-200 bg-white p-1.5 text-slate-500 hover:bg-slate-50 transition-colors">
                        <Bell class="size-4" />
            ...
            <!-- Page header slot -->
            <div v-if="subtitle || $slots.actions" class="shrink-0 border-b border-slate-100 bg-white px-5 py-3.5 flex items-center justify-between gap-4">
                <div>
...
.fade-text-enter-active { transition: opacity .1s, width .15s; }
.fade-text-leave-active { transition: opacity .08s; }
.fade-text-enter-from, .fade-text-leave-to { opacity: 0; width: 0; overflow: hidden; }
...
.slide-sidebar-enter-active, .slide-sidebar-leave-active { transition: transform .2s; }
