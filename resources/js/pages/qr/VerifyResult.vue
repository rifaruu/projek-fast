...
<template>
    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 to-emerald-50/20 flex items-center justify-center px-4 py-6">
        <div class="w-full max-w-sm space-y-3">
            ...
            <div
                class="overflow-hidden rounded-2xl bg-white shadow-lg ring-1"
                :class="{
            ...
            >
                ...
                <div v-if="found && surat" class="px-4 py-4 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <div class="col-span-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5">
                            <p class="text-[9px] font-semibold uppercase tracking-wider text-slate-400">Disahkan Oleh</p>
                            <p class="mt-0.5 text-[10px] font-bold text-slate-800">{{ surat.disahkan_oleh ?? '-' }}</p>
                        </div>
                        <div class="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5">
                        ...
                        <div class="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5">
                            <p class="text-[9px] font-semibold uppercase tracking-wider text-slate-400">Nama Approver</p>
                            <p class="mt-0.5 text-[10px] font-bold text-slate-800">{{ surat.nama_approver ?? '-' }}</p>
                        </div>
                        <div class="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5">
                            <p class="text-[9px] font-semibold uppercase tracking-wider text-slate-400">Tanggal Persetujuan Final</p>
                            <p class="mt-0.5 text-[10px] font-bold text-slate-800">{{ fmt(surat.tanggal_persetujuan_final) }}</p>
                        </div>
                    ...
                    >
                        <div class="grid size-6 shrink-0 place-items-center rounded-lg bg-emerald-500 text-xs font-bold text-white">✓</div>
                        <p class="text-xs font-medium text-emerald-800">Dokumen sah dan diterbitkan resmi oleh FMIKOM.</p>
                    </div>
                    <div
                        v-if="state === 'revoked'"
                        class="flex items-center gap-2.5 rounded-xl border border-red-200 bg-red-50 px-3 py-2.5"
                    >
                        <div class="grid size-6 shrink-0 place-items-center rounded-lg bg-red-500 text-xs font-bold text-white">✕</div>
                    ...
                    >
                        <div class="grid size-6 shrink-0 place-items-center rounded-lg bg-amber-400 text-xs text-white">⏳</div>
                        <p class="text-xs font-medium text-amber-800">Dokumen masih dalam proses persetujuan.</p>
                    </div>
                </div>

                <div v-if="!found" class="px-4 py-4">
                    <div class="space-y-1.5 rounded-xl border border-slate-100 bg-slate-50 px-4 py-3">
                ...
                <div class="flex items-center justify-between border-t border-slate-100 bg-slate-50/80 px-4 py-2.5">
            ...
            <div class="text-center">
                <a
                    href="/verifikasi-qr"
                    class="text-xs font-medium text-emerald-600 transition-colors hover:text-emerald-700 hover:underline"
                >
