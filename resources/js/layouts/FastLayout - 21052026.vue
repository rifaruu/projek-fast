...
// resources/js/layouts/FastLayout.vue
import { Head, Link, router, usePage } from '@inertiajs/vue3';
import { computed, ref, onMounted, onUnmounted, watch } from 'vue';
import {
    LayoutDashboard, FilePlus2, ClipboardList,
    Archive, LogOut, Bell, ChevronRight, Menu, X,
    History, FileText, Download, User,
...
const isDosen = computed(() => userSlug.value.includes('dosen'));
...
const navGroups = computed(() => [
    ...
    },
    {
        title: 'Surat',
        items: [
            { key: 'submit', label: 'Ajukan Surat', href: '/fast/user/dashboard#ajukan', icon: FilePlus2 },
            { key: 'history', label: 'Riwayat Surat', href: '/fast/user/history', icon: History },
            { key: 'archive', label: 'Arsip & Unduh', href: '/fast/user/archive', icon: Download },
        ] as NavItem[],
    },
    {
        title: 'Akun',
        items: [
            { key: 'profile', label: 'Profil Saya', href: '/settings/profile', icon: User },
        ] as NavItem[],
    },
]);
            ...
            <aside v-show="showSidebar"
                ...
                <nav class="flex-1 overflow-y-auto overflow-x-hidden px-2 py-2">
                    <template v-for="group in navGroups" :key="group.title">
                        ...
                        <Link v-for="item in group.items" :key="item.key"
                            :href="item.href"
        ...
        <div class="flex min-w-0 flex-1 flex-col overflow-hidden">
            ...
            <header class="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 lg:px-5">
                    ...
                    <nav class="hidden items-center gap-1 text-sm md:flex">
                        ...
                        <template v-if="props.breadcrumbs?.length">
                            <template v-for="(bc, i) in props.breadcrumbs" :key="i">
                                <Link v-if="bc.href" :href="bc.href"
                ...
                <div class="flex items-center gap-2">
                    <div class="hidden items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700 sm:flex">
                        {{ userRole }}
                    </div>
                </div>
            </header>
