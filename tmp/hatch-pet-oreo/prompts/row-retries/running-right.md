Create Codex pet row `running-right` for `oreo`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Black-and-white cat pet, rounded head, expressive eyes, white muzzle and chest, black ears and back, compact whole-body silhouette, no accessories, mascot-safe, readable at small size.. Preserve silhouette, face, palette, material, proportions, markings, and props.
